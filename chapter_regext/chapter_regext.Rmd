...
1\. From your ***computer*** `r emo::ji("backhand_index_pointing_right")` connect to [***www.wooclap.com/SCPOMLR***](https://app.wooclap.com/SCPOCAUSALITY)
...
Let's go back our [grades]("https://www.dropbox.com/s/wwp2cs9f0dubmhr/grade5.dta?dl=1") dataset. Remember that to load the data you need to use the `read_dta()` from the `haven` package. These are the estimates we got from regressing average math test scores on the full set of regressors.
...
1. Create a new variable `avgmath_stand` equal to the the standardized math score. You can use the `scale()` function or do it by hand with base `R`.
...
data <- data.frame(x = runif(100000, min = 0, max = 10000))
...
| Log - Level | $log(y) = b_0 + b_1 x + e$ | .small[A **one unit** increase in ] $x$ .small[ is associated, on average, with a] $e^{b_1}$ .small[ **percent change** in y]  |
...
| Log - Log  |  $log(y) = b_0 + b_1 log(x) + e$ | .small[A  ] ** $\Delta x$** .small[**percent** increase in ] $x$ .small[ is associated, on average, with a] $(1 + \Delta x)^{b_1}$ .small[**percent change** in y]  |

* Note that if $\Delta x = 5\% = 0.05 \implies 1 + \Delta x = 1.05$ 

--

* This may look like cooking recipes but of course it can be [derived with some calculus](https://www.youtube.com/watch?v=aXBFKKh54Es). Also check out [this great explainer](https://stats.idre.ucla.edu/other/mult-pkg/faq/general/faqhow-do-i-interpret-a-regression-model-when-some-variables-are-log-transformed/) for more info.

---

# Log Models: Simplified Interpretations

3 simplifications, which are used extremely often in practice:

--

***Log-Level***

When *** $b_1$ is small*** you can interpret using $1+b_1$ rather than $e^{b_1}$. This is because, for small $b_1$, $e^{b_1} \approx 1+ b_1$

--

$\rightarrow$ for $b_1 = 0.04$, $e^{b_1} = `r round(exp(0.04),4)`$ 

--

$\rightarrow$ for $b_1 = 0.5$, $e^{b_1} = `r round(exp(0.5),4)`$
...
For ***(relatively) small increases in $x$, $\delta x$***, you can interpret using $b_1 \times \delta x$ rather than $b_1 \times log(1 + \delta x)$, since for small $\delta x$, $log(1 + \delta x) = \delta x$

--

$\rightarrow$ for $\delta x = 1\%=0.01$, $log(1+\delta x) = `r round(log(1.01),2)`$

--

$\rightarrow$ for $\delta x = 20\%=0.20$, $log(1+\delta x) = `r round(log(1.20),2)`$
...
For ***small increases in $x$, $\delta x$, and small $|b_1|$***, you can interpret using $1 + b_1 \times \delta x$ rather than $(1 + \delta x)^{b_1}$, since for small $|b_1|\delta x$, $(1 + \delta x)^{b_1} \approx 1 + b_1 \times \delta x$

--

$\rightarrow$ for $\delta x = 1\%=0.01$ and $b_1 = 0.5$, $(1+\delta x)^{b_1} = `r round(((1.01)^0.5),2)`$

--

$\rightarrow$ for $\delta x = 10\%=0.10$ and $b_1 = 10$, $(1+\delta x)^{b_1} = `r round(((1.10)^10),2)`$

---

# Log Models: Simplified Interpretations Recap

For ***(relatively) small increases in $x$, $\delta x$***:

|    Specification           | Model  |  Interpretation of $b_1$            |
|--------------------|:---------:|:-----------------------------------:|
| Level - Level | $y = b_0 + b_1 x + e$ | .small[A **one unit** increase in ] $x$ .small[ is associated, on average, with a ] $b_1$ .small[**unit change** in y]  |
| Log - Level | $log(y) = b_0 + b_1 x + e$ | .small[A **one unit** increase in ] $x$ .small[ is associated, on average, with a] $1 + b_1$ .small[ **percent change** in y]  |
| Level - Log | $y = b_0 + b_1 log(x)  + e$ | .small[A  ] ** $\delta x$** .small[**percent** increase in ] $x$ .small[ is associated, on average, with a ] $b_1 \times \delta x$  .small[**unit change** in y] |
| Log - Log  |  $log(y) = b_0 + b_1 log(x) + e$ | .small[A  ] ** $\delta x$** .small[**percent** increase in ] $x$ .small[ is associated, on average, with a] $1 + b_1 \times \delta x$ .small[**percent change** in y]  |
...
2. Create a scatter of estimated mid career pay $(y)$ as a function of out of state tuition $(x)$. Would you say the relationship is broadly linear or rather non-linear? Use `geom_smooth(method = "lm", se = F) + geom_smooth(method = "lm", se = F, formula = y ~ poly(x, 2, raw= T))` to fit both a linear and 2nd order line. This time which seems most appropriate?
