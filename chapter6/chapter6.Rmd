...
* Because we can't do this activity in class, we drew 18 samples of 20 fusilli (with replacement) at home.
...
1. Create a vector containing the proportions of green pasta from the previous slide. Name it `pasta`. (Hint: remember that vectors are created using the `c()` function.)
...
pasta_samples %>%
  ...
  geom_histogram(boundary = 0.3, binwidth = 0.05, col = "white", fill = "darkgreen") +
  labs(
    x = "Proportion of green pasta",
    y = "Frequency"
  ) +
...
df %>% 
  ggplot(aes(x = prop_green)) +
    geom_histogram(binwidth = 0.05, boundary = 0.4, color = "white", fill = "darkgreen") +
...
* 2 key concepts relating to sampling for estimation:
  ...
  1. The effect of *sampling variation* on our estimates : how different our estimate would have been with another sample? 
...
df %>% 
  ...
  facet_wrap(~sample_n, labeller = as_labeller(
      c(`25` = "1000 samples of size n = 25",
          `50` = "1000 samples of size n = 50",
          `100` = "1000 samples of size n = 100"))) +
  theme_bw(base_size = 14) +
  geom_vline(xintercept = p, col = "black", size = 0.75) +
  annotate("text", x = 0.655, y = 410, label = "True population proportion", size = 3)
```
