...
# Quick "Quiz" on Last Week's Material | [<span style="color:blue">click here to start</span>](https://app.wooclap.com/LDIGLJ/questionnaires/5ff5d20d6a02a1111d3915bc)

`r countdown(left = 0, right = 0, bottom = 0, top = 0, padding = "50px", margin = "15%", font_size = "8em", minutes = 5, seconds = 0)`
...
* In the next two lectures you will learn to the basics of ***tidying***, ***visualizing*** and ***summarizing*** data
...
* Here are the first 3 rows and last 4 rows.
    ```{r}
    head(gapminder, n = 3)
    tail(gapminder, n = 4)
    ```
...
* It's important to know how the data is stored. We can use `str` for that:
    ```{r}
    str(gapminder)
    ```
...
1. Which countries had a 2011 GDP per capita greater than 30.000? 

1. Filter the dataset to only keep the year 2015 and assign it to a new object `gapminder_2015`.

1. How many countries have an infant mortality in 2015 greater than 90 (per 1000)?

1. What is the average life expectancy in Africa in 2015?
...
1. `summarise()`: Collapse to a single summary
...
# `group_by` + `summarize` = statistic by group

* Often we want to do *some* operation **by** some group in our dataset:
...
--
    2. Apply the operation to each group
--
    3. Recombine all groups into one table
--
    
* This is achieved by using `group_by()` and `summarise`.

---

# `group_by` + `summarize` = statistic by group

* `group_by(polls_us_election_2016, grade)` groups/splits `polls_us_election_2016` by pollster `grade`:
    ```{r}
...
* `summarise` each chunk and re-combine
  ```{r}
  head(summarise(polls_grade, mean_vote_clinton = mean(rawpoll_clinton)))
  ```
...
Can't we combine the `group_by` and `summarize` commands in one command?

--

Yes! Simple: use the `pipe` operator `%>%`:
...
*For the following questions you should use `mutate()` as well as `filter()`.*
...
* There is an extremely powerful alternative in package `ggplot2` (part of the `tidyverse` suite). We'll see both!
