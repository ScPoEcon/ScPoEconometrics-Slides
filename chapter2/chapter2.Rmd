...
```{r}
gapminder_2015 <- gapminder_2015 %>%
  mutate(infertility_above_2 = (gapminder_2015$fertility > 2.1)) # dummy variable for fertility rate above replacement level fertility
...
1. Compute the mean of `infant_mortality` in 1960? Read the help for `mean` to remove `NA`s.

1. Compute the median of `infant_mortality` in 1960. Is it greater or smaller than the average?

1. Create of density plot using `geom_density` of `infant_mortality` in 1960. Add a `geom_stat` contaning the average and another containing the median. What do you observe?

1. Compute the correlation between infant mortality and gdp per capita in 2015.

Optional in your free time, you can do this tutorial:
```{r, echo = TRUE, eval=FALSE}
library(ScPoApps)
