...
* Run the same OLS regression by yourself but for the verbal score.

* Does the slope coefficient close to the one of math score? Was it expected?

* Prediction : What is the expected score when the class size is equal to 30? 
...
* Standardizing a variable means to demean and to divide the demeaned value by its own standard deviation. 
$$ \widetilde z = \frac{z - \bar z}{sd(z)} $$

* But why would we standardize the variables in the regression?

--

* Imagine a **beauty contest** where judges take into account **weight** (measured in *kilograms*) and **height** (measure in *meters*). You are asked to combine them into a single measure. 

* Simply taking the average of the two would not be very informative because the variations in height would be completely diluted by the variations in weight. 

* Standardizing the two variables first would solve this problem. 

---

class: inverse

# Task : Standardized OLS

* Add 
    * 2 variables (`std_math` and `std_verb`) that gives both the standardized math and verbal score.
 
    * a new variable `std_score` that associate to each class size a unique score.

    * a new variable `std_classize` that is the standardized class size. 


* Run the OLS regression of the `std_score` on the `std_classize`. 

* Interpret the value of each coefficient. 

---

# Other OLS variations / restrictions

* All are described [in the book](https://scpoecon.github.io/ScPoEconometrics/linreg.html#OLS). Optional emo::ji("nerd_face").
...
* Interpretation
    ...
    * Compute the predicted wage associated to 15 years of education.
