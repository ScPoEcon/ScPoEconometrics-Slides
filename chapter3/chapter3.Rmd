...
* We call the value $\widehat{y}_i$ the *predicted value*, after we choose values for $b_0,b_1$ and $x$.
    ...
    $$
* In general, we expect to make **errors** in our prediction.
]
...
covs = data.frame(dataset = 1:4, cov = 0.0, vary = 0.0, varx = 0.0)
for(i in 1:4) {
  ...
  covs[i,"vary"] = eval(parse(text = paste0("var(anscombe$y",i,")")))
  covs[i,"varx"] = eval(parse(text = paste0("var(anscombe$x",i,")")))
}
