...
* R basics : first steps with R studio, importing data, data handling 
...
Let's start with the relation between **student performance and class size**.

The data we gonna use consists of test scores in fifth grade classes at public elementary schools in Israel. These data were originally used in [Angrist and Lavy (1999)](https://www.dropbox.com/s/le8c38jrzma656t/angrist-lavy.pdf?dl=0)

 **TASK for you**
 
* First, download the data from [here](https://www.dropbox.com/s/2g16pid3eky44bp/grades_classize.RData?dl=0) and open it in your R session.
...
grades = grades %>% 
  group_by(classize) %>%
  summarise(avgmath_by_class= mean(avgmath), 
            avgread_by_class= mean(avgverb), 
            avggrade_by_class= (avgread_by_class+avgmath_by_class)/2)
```

* Describe the dataset : What are the variables? What is an observation here? 

* Create a new dataset with both the average math and verbal grade by class size (*i.e. the average over multiple class that have the same class size*).

* Add a new variable that summarise both grades for each class size (*simple solution : the mean*).

* How do the grades seem to be related to the size of the class? 
...
plot(avggrade_by_class ~ classize, data = grades,
     xlab = "Class size",
     ylab = "Average grade",
     main = "Average grade by class size",
     pch  = 20,
     cex  = 2,
     col  = "red")
```
...
plot(avggrade_by_class ~ classize, data = grades,
     xlab = "Class size",
     ylab = "Average grade",
     main = "Average grade vs class size ",
     pch  = 20,
     cex  = 2,
     col  = "red")
...
* We simply and summarise this relationship with a line (for now).
...
grades %>% 
  ggplot(aes(x=classize,y=avggrade_by_class)) + 
  geom_point(col = "red") + geom_smooth(method = "lm", se = F) + 
  labs(x = "class size",y = "Average grade") +
  theme_classic()
```
]
...
```{r echo=T, eval = F}
reg = lm(avggrade_by_class~classize,grades) # assign the result of the regression to the "reg" object 
...
```{r echo=FALSE}
coeffs = lm(avggrade_by_class~classize,grades)$coefficients
coeffs
```
