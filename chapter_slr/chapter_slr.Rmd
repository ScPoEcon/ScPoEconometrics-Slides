...
## Class size and student performance => create quick zoom poll about interpretation
...
1. Compute the OLS coefficients $b_0$ and $b_1$ of the previous regression using the formulas on slide 24. (*Hint:* you need to use the `cov`, `var`, and `mean` functions.)
...
# Task 4: $R^2$ and goodness of fit
...
1. Install and load the `broom` package. Pass `math_reg` in the `augment()` function and assign it to a new object. Use the variance in `avgmath_cs` (SST) and the variance in `.fitted` (predicted values; SSE) to find the $R^2$ using the formula in the previous slide.
