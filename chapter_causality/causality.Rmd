...
- Based on [Cochran (1968)](https://www.jstor.org/stable/2528036), we will use death rates in Canada, the U.K. and the U.S.
...
`r emo::ji("warning")` not all students benefitted equally from the treatment!
...
$$
\begin{align}
\color{red}{\text{ATT}} &= \mathbb{E}(\delta_i | D_i = 1) \\
&\approx 2.71
\end{align}
$$
...
$$
\begin{align}
\color{red}{\text{ATT}} &= \mathbb{E}(\delta_i | D_i = 0) \\
&= -4
\end{align}
$$
...
baseline_graph <- star_df_k_small %>%
    ...
    scale_x_continuous(lim = c(-.5,1.5), breaks = c(0,1), labels = c("FALSE","TRUE")) +
