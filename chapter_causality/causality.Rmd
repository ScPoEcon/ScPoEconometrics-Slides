...
- The key point here is the ***without changing anything else***, often referred as the **ceteris paribus assumption**   
...
- In each study 3 groups of men have been followed from 20 months (U.S.) to 5 five years (U.K.)
...
- Death rates are 30% (U.S.) to 75% (Canada) higher among cigar/pipe smokers than among non-smokers.
...
This table replicates, for Canada, the 3 age group distribution of pipe-smokers and non-smokers, and death rate of pipe-smokers.

1) Compute the average death rate for pipe smokers in Canada from age grouped death rates?

2) What would be the average mortality rate be for pipe smokers if they had the same age distribution as the non-smokers?
...
* From these we can compute the ***individual treatment effect***:
...
.footnote[
This image is from Morgan and Winship (2015, p.46).
]
---
...
* The ATT measures the ***average treatment effect conditional on being in the treatment group***.
  ...
  * The $\mathop{\mathbb{E}}(.|C)$ operator is also linear.
...
* Notice that this kind raw comparison is often done by journalists, politicians, badly trained scientists.

---

# The Problem of Causal Inference

Let's rewrite the SDO, to make the ATE $(\delta_i)$ appear in the equation. 
...
* **Intuition** : Suppose that *education* is our treatment and this is people who will most benefit from school (higer $\delta_i$) who stay in school longer. Then, simply comparing outcomes between highly educated and low educated would ovestimate the effect of schooling among the whole population. 
...
Let's compute these various quantities and bias with some toy data (i.e. data we generated ourselves).
...
# 
# 
# toy_data$Y_i = toy_data$D_i*toy_data$Y1_i + (1-toy_data$D_i)*toy_data$Y0_i
# toy_data$Y_i_random = toy_data$D_i_random*toy_data$Y1_i + (1-toy_data$D_i_random)*toy_data$Y0_i
# toy_data$delta_i = toy_data$Y1_i - toy_data$Y0_i
# 
# 
# ATE = mean(toy_data$delta_i)
# 
# SDO = mean(toy_data$Y_i[toy_data$D_i==1]) - mean(toy_data$Y_i[toy_data$D_i==0]) 
# SDO_random = mean(toy_data$Y_i_random[toy_data$D_i_random==1]) - mean(toy_data$Y_i_random[toy_data$D_i_random==0]) 
# 
# (SDO - ATE)
# (SDO_random - ATE)
# 
# 
# selection_bias = mean(toy_data$Y0_i[toy_data$D_i==1]) - mean(toy_data$Y0_i[toy_data$D_i==0]) 
# het_trt_effect_bias = (1 - sum(toy_data$D_i==1)/nrow(toy_data))*(mean(toy_data$delta_i[toy_data$D_i==1]) - mean(toy_data$delta_i[toy_data$D_i==0]))
# 
# (SDO == ATE + selection_bias + het_trt_effect_bias)
```

1) Load the data [here](https://www.dropbox.com/s/a4u67nmu9gnl94y/toy_data.csv?dl=1). Notice that `Di_random` is a treatment status we would have under a given random assignment.

2) Compute and add $Y_i$ and $\delta_i$ as new variables. 

3) Compute the **ATE** and the **SDO**. Is there is any bias? Is it large in magnitude? 

4) Using `D_i_random`, compute the **SDO under randomization**. If you got it right, the bias should be almost 0 (about 0.017 actually). 

5) Bonus :  Compute the value of the **selection bias** and the **heterogenous treatment effect bias** and check that we have $$SDO = ATE + \textrm{selection bias} + \textrm{heterogenous treatment effect bias}$$ 
...
- The first RCTs were conducted, long time ago (18th and 19th century), mainly in **Medecine**. 

- In the beginning of the 20th century they have popularized by famous statisticians like **J. Neyman** or **R.A. Fisher**. 

- Since then they have been a growing influence and progressively accepted as a [tool for public policy](https://www.povertyactionlab.org/fr). 

- As for Economics, the **Nobel Price in Economics 2019** has been awarded to three exponents of the RCT literature, [Duflo, Banerje and Kremer](https://www.economist.com/finance-and-economics/2019/10/17/a-nobel-economics-prize-goes-to-pioneers-in-understanding-poverty).
...
Let's only focus on one treatment group: **small classes**; and on one grade: **kindergarten** (k).
...
$$
D_i = \begin{cases} 
                    1 \textrm{  if small class}  \\\\ 
                    0 \textrm{  if regular class} 
      \end{cases}
$$
```{r, echo = FALSE}
...
lm(math ~ star, star_df_k_small)
```

--

Compare with results from the table a few slides back!
...
$\begin{align} \mathbb{E}[\textrm{math score} | D_i = \textrm{regular class}]&= \mathbb{E}[b_0 + \delta D_i + e_i | D_i = \textrm{regular class}] \\ &= b_0 + \delta \mathbb{E}[D_i| D_i = \textrm{regular class}] \\ &= b_0 \end{align}$

$\begin{align} \mathbb{E}[\textrm{math score} | D_i = \textrm{small class}]&= \mathbb{E}[b_0 + \delta D_i + e_i | D_i = \textrm{small class}] \\ &= b_0 + \delta \mathbb{E}[D_i| D_i = \textrm{small class}] \\ &= b_0 + \delta \end{align}$

$\begin{align} ATE &= \mathbb{E}[\textrm{math score} | D_i = \textrm{small class}] - \mathbb{E}[\textrm{math score} | D_i = \textrm{regular class}] \\ &= b_0 + \delta - b_0 \\ &= \delta \end{align}$
...
Notice from the regression output that only one of the outcomes is included in the regression!

```{r, echo = FALSE, eval = TRUE}
lm(math ~ star, star_df_k_small)
```

This is because $D_i$, the treatment variable, is either small class size or regular class. The coefficient for regular class is "captured" by the intercept.
...
1. Compute the average math score for both groups.

1. Regress the treatment dummy variable on math score. Are the results in line with the previous question?

1. Go back to the original data and this time keep first graders but don't get rid of the regular+aide treatment group.

1. Compute the average math score for the regular+aide treatment group.

1. Regress the treatment dummy variable on math score. What do you observe? How do you interpret the coefficients? What's the ATE of small class size compared to regular+aide?
...
* **Interpretation** of the results
  ...
  * Imperfect randomization issues, attrition,...
