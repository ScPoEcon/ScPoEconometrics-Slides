...
* *Intuitive check:* compare trends before policy change (after change if no expected medium-term effects)
...
* the parallel trend is about what happens between period 1 and 2, the pre-period evolution is a common trend assumption check!

* graph with possible what would have happened absent policy changes in NJ showing the bias is ATE
...
* In practice, DiD are usually estimated on more than 2 periods (4 observations)
...
0 \quad \text{if } s = \text{New Jersey} \\\
1 \quad \text{if } s = \text{Pennsylvania}
\end{array}\end{cases}
$$
...
# Multi-period DiD: Relaxing Common Trends Assumption
