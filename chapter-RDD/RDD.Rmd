...
$$\underbrace{\lim_{c \to 21^+} E[DEATHRATE_a^1|a = c]}_{\alpha + \delta} - \underbrace{\lim_{c \to 21^-} E[DEATHRATE_a^0|a = c]}_{\alpha} = \delta$$
...
1. Estimate the following model on all death causes, where $a$ is defined in months *relative* to the 21st birthday
$$DEATHRATE_a = \alpha + \delta D_a + \beta a + \varepsilon_i,$$
...
$$DEATHRATE_a = \alpha + \delta D_a + \beta a + e_a,$$
...
--
   ...
   $\rightarow$ an overly flexible specification reduces precision and runs the risk of overfitting.
