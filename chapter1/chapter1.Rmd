...
1. Ensure that `murders` is a data.frame by running: `class(murders)`
    ```{r,eval=F}
...
1. Find out what variables are contained in `murders` by running:
    ```{r, eval = F}
    names(murders) # obain variable names
    ```
...
I will ***exclusively*** use Slack for our interactions.

Please ***DO NOT*** contact me by email (unless there's a legitimate reason to).

I will be checking Slack once around 14h and once around 18h.
]
...
*Work in groups:* You can work in groups of 2 (3 max) on the quizzes.
]
...
To combat everyone's social isolation (mine included!), I propose we meet weekly on [gather.town](https://gather.town/app) every Friday at 17h `r emo::ji("hugs")`
