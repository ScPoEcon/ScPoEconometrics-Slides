...
[<ru-blockquote>
Does higher public debt levels *lead to* lower economic growth?
...
* You can (relatively) easily produce nice looking maps! You will create your own in the midterm `r emo::ji("ok_hand")`

.pull-left[

```{r iris_map, eval = FALSE}
library(sf)
library(tidyverse)

iris_2021 <- read_sf("code/contours_iris_2021/CONTOURS-IRIS.shp")
median_inc_paris <- read.csv("code/median_inc_paris.csv", colClasses = ("IRIS" = "character"))

iris_2021_paris <- iris_2021 %>%
    mutate(dep = substr(INSEE_COM, 1, 2)) %>%
    filter(dep == "75")
iris_map <- left_join(iris_2021_paris, median_inc_paris)

iris_map %>%
    ggplot(aes(fill = median_inc_bins)) +
    geom_sf(size = .25) +
    scale_fill_manual(values = c("#FFFFCC", "#F6D868", "#E79652", "#CD524C", "#70372D", "#191900", "grey")) +
    labs(fill = "Median household income (euros)") +
    theme_void() +
...
```{r iris_map_out, ref.label = "iris_map", echo = FALSE}
```
]
...
1. Create a `total_percap` variable equal to the number of murders per 10,000 inhabitants by running the follwoing code.
    ```{r}
