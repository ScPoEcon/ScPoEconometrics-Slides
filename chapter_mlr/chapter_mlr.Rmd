...
\begin{align}
\mathbb{E}(\text{average math score} | \text{religious} = 1 \text{ & } \text{class size} \neq 0) &= \color{#d96502}{b_0} + \color{#d90502}{b_1} \times \text{class size} + \color{#027D83}{b_2} \times 1 \\
...
\begin{align}
\mathbb{E}(\text{average math score} | \text{religious} = 0 \text{ & } \text{class size} \neq 0) &= \color{#d96502}{b_0} + \color{#d90502}{b_1} \times \text{class size} + \color{#027D83}{b_2} \times 0 \\
...
\begin{multline}
\mathbb{E}(\text{average math score} | \text{religious} = 1 \text{ & } \text{class size} \neq 0) - \\ \mathbb{E}(\text{average math score} | \text{religious} =0 \text{ & } \text{class size} \neq 0) \\
...
$\rightarrow$ $\color{#027D83}{b_2}$ corresponds to the expected difference in average math score between religious and none religious schools, keeping class size constant
...
\begin{multline}
\color{#027D83}{b_2} = \mathbb{E}(\text{average math score} | \text{religious} = 1 \text{ & } \text{class size} \neq 0) - \\ \mathbb{E}(\text{average math score} | \text{religious} =0 \text{ & } \text{class size} \neq 0)
\end{multline}
...
graph_reg_b0 <- graph_reg +
    ...
    scale_y_continuous(breaks = c(30,50,b0,70,90), labels = c("30","50",parse(text = TeX("$b_0$")),"70","90"), minor_breaks = seq(30,90,20)) +
...
This condition is particularly for ***categorical variables***, i.e. variables that take a limited of possible "levels" (e.g. gender, seasons, race, education levels, etc.)
...
$\rightarrow$ `R` will automatically detect perfect collinearity between variables and drop of them

--

`r emo::ji("warning")` you have to pay attention to the ***reference category***: the "baseline" category from which the coefficients are interpreted

--

This applies to cases when you have more than 2 categories. You don't need to create a dummy variable for each possibility, `R` will detect that the categorical variable and do that for you.

--

***But*** you have to look which category has been chosen as the ***reference category***.
...
From this formula you obtain both the ***magnitude*** of the OVB and its ***sign*** (positive/negative).
