...
* Let's see hw it works taking back the **STAR experiment data**. 

* In particular we will focus on how the effect of being in a small vs regular class vary with the experience of the teacher. 

* Our regression model is 

$$ \textrm{score_i} = b_0 + b_1 \textrm{SMALL}_i + b_2 \textrm{experience}_i + b_3 \textrm{SMALL}_i * \textrm{experience}_i + e$$ 

* What is the predicted test score of a student in a:

  * .small[small class with a 10 years experienced teacher:] $\hat{\textrm{score}_i} = b_0 + b_1 + b_2*10 + b_3 * 10$
  
  * .small[regular class with a 10 years experienced teacher:]: $\hat{\textrm{score}_{i'}} = b_0 + b_2*10$
  
  * .small[The difference in the predicted score for a student in small vs regular class with a 10 years experienced teacher is then:] $b_1 + b_3*30$

---

# Interacting regressors

Running the regression for the `avgmath` score, we obtain: 

```{r, echo=FALSE}
star_df = read.csv("../../../../../../Dropbox/ScPoEconometrics/pierre_gustave/star_data.csv")
...
```{r}
round(lm(math ~ SMALL + experience + SMALL*experience, star_df)$coefficients,3)
```
...
--
  ...
  * Thanks to our interaction term, we let the impact of being in a small class vary with the experience of the teacher. 
...
```{r,echo=F, fig.height=3.8, fig.width=6}
ggplot(star_df, aes(x = experience, y = math, group = SMALL, color = SMALL)) +
    geom_smooth(method = "lm", se = F) +
    theme_bw() +
  labs(title = "Mathematics test score")
```

* It gives you the predicted value of student's math score at any level of teacher experience depending on the class size: small or regular. 

---

# Interacting regressors

Here is the same visualization broken down by grades. 
